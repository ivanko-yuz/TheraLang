...
export class GeneralResourcesInnerTableComponent implements OnInit {
  ...
  displayedColumns: string[] = ["name", "date", "description", "url"];
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;

  constructor(public resourceService: ResourceService) {}

  async ngOnInit() {
    this.resources = await this.resourceService.getResourcesByCategoryId(
      this.resourcesCategoryId,
      Constants.ResourcesTableConstants.PAGE_NUMBER,
      Constants.ResourcesTableConstants.COLUMNS_PER_PAGE
    );

    this.allResourcesCount = await this.resourceService.getResourcesCountByCategoryId(
      this.resourcesCategoryId
    );
